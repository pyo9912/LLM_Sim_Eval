...
if __name__ == '__main__':
    ...
    dialog_id2data = {}
    log_path = os.path.join(args.home, f'save_{args.turn_num}/chat/{args.interaction_model}/{args.dataset}/{args.topk}/{args.interaction_log}')
    
    if os.path.exists(log_path) and len(os.listdir(log_path)) > 0:
        path_list = os.listdir(log_path)
        print(f"turn_num: {args.turn_num}, model: {args.interaction_model} dataset: {args.dataset}", len(path_list))
        
        for path in tqdm(path_list):
            with open(f"{log_path}/{path}", 'r', encoding="utf-8") as f:
                data = json.load(f)
                dialog_id = data['dialog_id']
                dialog_id = str(dialog_id) + '_' + str(data['turn_id'])
                dialog_id2data[dialog_id] = data  
    ...
    while len(dialog_id_set) > 0:
        ...
        # 추가되는 부분
        data['context'] = data['context'] + [i['content'] for i in data['simulator_dialog']['context'][data['simulator_dialog']['turn_id']:-2]]
        data.pop('simulator_dialog', None)
        ...
        for i in range(0, args.turn_num):
            ...
            seeker_full_response = """1. Recommender's intent: recommendation
            2. Response: Thank you for your recommendation."""
